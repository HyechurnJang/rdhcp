...
class NameSpace(Model):
    ...
    def __sync__(self):
        ...
        cli('ip link add rve_%s type veth peer name rve_%s netns %s' % (self.name, self.name, self.name))
        cli('ifconfig rve_%s 0.0.0.0 up' % self.name)
        cli('ip netns exec %s ifconfig rve_%s %s netmask 255.255.255.0 up' % (self.name, self.name, route_ip))
        cli('route add -host %s/32 dev rve_%s' % (route_ip, self.name))
        cli('iptables -A FORWARD -i rve_%s -j ACCEPT' % self.name)
        cli('ip netns exec %s route add -host %s/32 dev rve_%s' % (self.name, mgmt_ip, self.name))
        ...
        cli('ip netns exec %s iptables -t nat -A POSTROUTING -o rve_%s -j MASQUERADE' % (self.name, self.name))
    ...
    def __delete_namespace__(self):
        if self.pid: cli('ip netns exec %s kill -9 %d' % (self.name, self.pid), force=True)
        cli('ip link del rve_%s' % self.name, force=True)
        cli('ip netns del %s' % self.name, force=True)
        cli('rm -rf /opt/rdhcp/%s' % self.name, force=True)
        cli('iptables -D FORWARD -i rve_%s -j ACCEPT' % self.name, force=True)
        pygics.sleep(1)
